...
categories = {0:'back'}
label_file = open('label_map.pbtxt')
prev_id = 0
for line in label_file.readlines():
    if 'id' in line:
        line = line.replace('id:','')
        prev_id = int(line.strip())
    if 'display_name' in line:
        index_start = line.find('"')
        word = line[index_start + 1:-2]
        categories[prev_id] = word
...
class Object(object):
    def __init__(self):
        self.name="webrtcHacks TensorFlow   Detection REST API"
...
def get_objects(image, threshold=0.5):
    ...
    for c in range(0, len(classes)):
        class_name =  categories[classes[c]]#'abcd'#category_index[classes[c]]['name']
